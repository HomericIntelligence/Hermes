...
This file is the sole authoritative agent contract for this repository. `CLAUDE.md` is a pointer
here.
...
Every NATS message body is a UTF-8 JSON object (defined by `HermesEventBase` in
...
These services are verified in this document and the repository architecture:
...
9. **Stateless HTTP layer.** Hermes holds no state; NATS JetStream is the event history.

10. **Fail fast on invalid payloads.** HMAC validation and Pydantic parsing reject bad data at
    the boundary.

11. **Subject granularity.** Fine-grained subjects let subscribers filter precisely.

12. **Reproducible Docker builds.** The Docker image installs runtime deps from
    `requirements.lock.txt` with `pip install --require-hashes`. The lock file is generated by
    `just lock` (pip-compile --generate-hashes) against Python 3.12 to match `python:3.12-slim`.
    Refresh it in any PR that changes `[project.dependencies]`. CI's `version-check` job enforces
    freshness.

13. **Pin dependency versions (detail).** Lower bound at the minor version level of the minimum
    supported version, upper bound at the next major to prevent breaking changes.
