#!/bin/bash
# Run the Hermes CI suite locally inside a container.
#
...
#
# Image: uses 'hermes-ci:local' if available, falls back to GHCR image.
# Build locally: just ci-build  (or: podman build -f ci/Containerfile -t hermes-ci:local .)
...
run_in_container() {
    ...
    # Docker-compatible socket passthrough: some pre-commit hooks (e.g.
    # spectral-openapi, docker-compose-config) invoke `docker` inside the CI
    # container. Under podman-by-default we expose the host's docker-compatible
    # socket (docker.sock, or the rootless podman socket aliased to it).
    if [ -S /var/run/docker.sock ]; then
        socket_mount="-v /var/run/docker.sock:/var/run/docker.sock:Z"
    elif [ -S "${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/podman/podman.sock" ]; then
        socket_mount="-v ${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/podman/podman.sock:/var/run/docker.sock:Z"
    fi
    # shellcheck disable=SC2086
    "${CONTAINER_ENGINE}" run --rm --userns=keep-id:uid=1000,gid=1000 $caches $socket_mount \
...
run_unit-tests() {
    ...
    run_in_container "pixi install --locked --quiet && pixi run pytest -m 'not integration' -q"
}
