...
def test_dockerfile_copies_requirements_before_pip_install() -> None:
    """COPY requirements.txt must appear before the RUN pip install line in the builder stage."""
    lines = _dockerfile_lines()
    copy_idx = next(
        (i for i, ln in enumerate(lines) if "COPY requirements.txt" in ln), None
    )
    pip_idx = next(
        (i for i, ln in enumerate(lines) if _PIP_INSTALL_RE.search(ln)), None
    )
    assert copy_idx is not None, "Dockerfile has no 'COPY requirements.txt' line"
    assert pip_idx is not None, "Dockerfile has no 'pip install' line"
    assert copy_idx < pip_idx, (
        "COPY requirements.txt must appear before RUN pip install in the Dockerfile"
    )


def test_dockerfile_pip_install_uses_requirements_file() -> None:
    """The pip install command must use requirements.txt (lock-pinned), not inline range extraction."""
    text = DOCKERFILE.read_text()
    assert "-r requirements.txt" in text, (
        "Dockerfile pip install must use -r requirements.txt (generated by scripts/sync_requirements.py)"
    )
